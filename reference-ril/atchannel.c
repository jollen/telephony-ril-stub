...
int at_pseudo_open(char *path)
{
    ...
    /* for pppd termio */
    ios.c_cflags &= ~(CSIZE|CSTOPB|PARENB);
    ios.c_cflags |= CS8|CREAD|CLOCAL;
    ios.c_iflags = IGNPAR;
    ios.c_oflags = 0;
    ios.c_lflags = 0;
    ...
    /* allocate pts number */
    ioctl(fd, TIOCGPTN, &pts_num);
    snprintf(pty_path, 64, "/dev/pts/%d", pts_num);
 
    memcpy(path, pty_path, 65);
