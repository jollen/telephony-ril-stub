...
{
    ATResponse   *p_response = NULL;
    int		 *muteRequest = ((int *)data)[0];
    char	 *cmd = NULL;
    int		 err;
    
    asprintf(&cmd, "AT+CMUT=%d", muteRequest);
    err = at_send_command(cmd, &p_response);
    free(cmd);
...
{	
    ...
    at_pseudo_open(pts_path);

    /* Write ppp options to pppd */
    fd = creat (PPP_OPTION_FILE, 0644);

    write(fd, pts_path, strlen(apn));
    write(fd, "\n", 1);

    write(fd, "ctscts", 6);
    write(fd, "\n", 1);

    asprintf(&options, "user %s\n", username);
    write(fd, options, strlen(options));
    free(options);

    asprintf(&options, "password %s\n", password);
    write(fd, options, strlen(options));
    free(options);
    ...
    /* Set PDP-type */
#define	PDP_TYPE_X25	1
#define	PDP_TYPE_IP	2
#define PDP_TYPE_OSPIH	3
#define PDP_TYPE_PPP	4
    asprintf(&options, "AT+CGDCONT=3,PDP_TYPE_PPP,\"%s\"\n", apn);
    at_send_command(options, NULL);  /* FIXME: */
    free(options);

    /* Set Qos (+CGQREQ, +CGQMIN) */
    at_send_command("AT+CGQREQ=3", NULL);  /* FIXME: */
    at_send_command("AT+CGQMIN=3", NULL);  /* FIXME: */
    /* Set event report (+CGEREP) */
    at_send_command("AT+CGERQP=1,0", NULL);  /* FIXME: */

    /* Dialing */
    at_send_command("ATDT*99***", &p_response);  /* FIXME: */
   
    char *response[2];
    ATLine *msg = p_response->p_intermediates);

    asprintf(&response[0], "%s", msg->line);
    line = msg->next;
    asprintf(&response[1], "%s", msg->line);
    
    RIL_onRequestComplete(t, RIL_E_SUCCESS, response, sizeof(response));
    at_reponse_free(p_response);
    free(response);
    return;
